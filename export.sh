...
executeExport(){
        ...
        [ -e ${OLD_OPENSHIFT_PROJECT}-is.json ] && rm ${OLD_OPENSHIFT_PROJECT}-is.json
        [ -e ${OLD_OPENSHIFT_PROJECT}-dep.json ] && rm ${OLD_OPENSHIFT_PROJECT}-dep.json
#       cd $OPENSHIFT_OC_HOME && unset KUBECONFIG && \
                ...
                ./oc project ${OLD_OPENSHIFT_PROJECT} && \
                 ./oc export is -o json >> ${OLD_OPENSHIFT_PROJECT}-is.json && \
                #./oc export dc -o json >> ${OLD_OPENSHIFT_PROJECT}-dep.json && \

                echo "===> export ${OPTIONS} " && \
                ./oc export ${OPTIONS} -o json >> ${OLD_OPENSHIFT_PROJECT}.json && \

                #echo "sed -i 's/${OLD_OPENSHIFT_PROJECT}/${NEW_OPENSHIFT_PROJECT}/g' ${OLD_OPENSHIFT_PROJECT}.json" && \
                sed -i "s/${OLD_OPENSHIFT_PROJECT}/${NEW_OPENSHIFT_PROJECT}/g" ${OLD_OPENSHIFT_PROJECT}.json && \
                #sed -i "s/${OLD_OPENSHIFT_PROJECT}/${NEW_OPENSHIFT_PROJECT}/g" ${OLD_OPENSHIFT_PROJECT}-dep.json && \
                ...
                sed -i '/\"volumes\"/,/]/ d; /^$/d' ${OLD_OPENSHIFT_PROJECT}.json
                sed -i '/\"volumeMounts\"/,/]/ d; /^$/d' ${OLD_OPENSHIFT_PROJECT}.json
                ...
                ./oc policy add-role-to-group system:image-puller system:serviceaccount:${NEW_OPENSHIFT_PROJECT} -n ${OLD_OPENSHIFT_PROJECT} && \
                ...
                #./oc create -f $(pwd)/${OLD_OPENSHIFT_PROJECT}-dep.json -n ${NEW_OPENSHIFT_PROJECT}
                ./oc create -f $(pwd)/${OLD_OPENSHIFT_PROJECT}-is.json -n ${NEW_OPENSHIFT_PROJECT}
                ./oc create -f $(pwd)/${OLD_OPENSHIFT_PROJECT}.json -n ${NEW_OPENSHIFT_PROJECT}
